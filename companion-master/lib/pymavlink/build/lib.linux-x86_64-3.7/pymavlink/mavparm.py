'''
module for loading/saving sets of mavlink _parameters
'''
...
class MAVParmDict(dict):
    def __init__(self, *args):
        ...
        # some _parameters should not be loaded from files
    ...
    def save(self, filename, wildcard='*', verbose=False):
        ...
        f.close()
        if verbose:
            print("Saved %u _parameters to %s" % (count, filename))


    def load(self, filename, wildcard='*', mav=None, check=True, use_excludes=True):
        '''load _parameters from a file'''
        try:
        ...
        f.close()
        if mav is not None:
            print("Loaded %u _parameters from %s (changed %u)" % (count, filename, changed))
        else:
            print("Loaded %u _parameters from %s" % (count, filename))
        return True
