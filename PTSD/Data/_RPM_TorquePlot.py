...
files = sorted(list_of_files, key=os.path.getctime, reverse=1)
print(files)
# index = int(input('File [0-{!s}]: '.format(len(files))))
...
for i in range(len(files)):
    ...
    width = 16.
    fig, (ax1) = plt.subplots(1, 1, sharex=True, figsize=(width, width/1.6))
    ...
    plt.savefig('./PNG/{!s}_Torque_RPM.png'.format(filename[:-4]), dpi=255, format='png')
    # plt.show()
